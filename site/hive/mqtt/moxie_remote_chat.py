...
def _first_content_id(cid: str) -> str:
    """Pick the first content id if multiple are delimited with '|'."""
    if not cid:
        return ""
...
class RemoteChat:
    """RemoteChat is the plugin to the MoxieServer that handles all remote module requests.

    It keeps track of the active remote module, creates new ones as needed, and ignores all data
    ...
    # Update database backed records. Query to get all the module/content IDs and update
    # the modules schema and register the modules
    def update_from_database(self):
        ...
        for chat in SinglePromptChat.objects.all():
            ...
            for content_id in cid_list:
                new_modules[f"{chat.module_id}/{content_id}"] = {
                    "xtor": SinglePromptDBChatSession,
                    "params": {"pk": chat.pk},
                }
        ...
        # Modules/content IDs into the module info schema - bare bones mandatory fields only
        mlist = []
    ...
    def on_chat_complete(self, device_id, id, session: ChatSession):
        ...
        if session.has_complete_hook():
            ...
            volley = Volley(
                ...
                device_id=device_id,
                data_only=True,
                robot_data=self._server.robot_data().get_volley_data(device_id),
                local_data=session.local_data,
            )
    ...
    def get_session(self, device_id, id, maker) -> ChatSession:
        ...
        if device_id in self._device_sessions:
            if self._device_sessions[device_id]["id"] == id:
                ...
                self.on_chat_complete(
                    device_id, self._device_sessions[device_id]["id"], self._device_sessions[device_id]["session"]
                )
    ...
    # Get a chat session object for use in the web chat
    ...
    # Get the next response to a chat — FINAL ONLY (no partials to the robot)
    def create_session_response(self, device_id, sess: ChatSession, volley: Volley):
        """Unified behavior for both OpenAI and Ollama:
        - Let the session build the prompt/context and get a full answer (no device partials).
        - Add automarkup if missing.
        - Send exactly one remote_chat response to the robot.
        """
    ...
    def handle_request(self, device_id, rcr, volley_data):
        ...
        # Accept either new keys (module_id/content_id) or legacy (module/content)
        module_id = (rcr.get("module_id") or rcr.get("module") or "")
        content_id = (rcr.get("content_id") or rcr.get("content") or "")

        id = f"{module_id}/{content_id}"
        if _LOG_NOTIFY_RCR and cmd == "notify":
            self.log_notify(rcr)

        maker = self._modules.get(id)

        # ---------- AUTO-MAP STT/ROUTER REQUESTS ----------
        # If no remote module matched AND this came from the router (speech),
        # redirect to a suitable chat.
        if not maker and rcr.get("backend") == "router" and cmd != "notify":
            rerouted_id = None

            # 1) Prefer existing device session (continue the current chat)
            sess = self._device_sessions.get(device_id)
            if sess:
                cand = sess.get("id")  # ex: "<module>/<content>"
            ...
            if not rerouted_id:
                for chat in SinglePromptChat.objects.all().order_by("-pk"):
                    ...
                    candidate_id = f"{chat.module_id}/{cid}"
                    if candidate_id in self._modules:
                        rerouted_id = candidate_id
                        break

            if rerouted_id:
                new_module, new_content = rerouted_id.split("/", 1)
                # write both key styles
                rcr["module_id"] = new_module
                rcr["module"] = new_module
                rcr["content_id"] = new_content
                rcr["content"] = new_content
                module_id, content_id = new_module, new_content
                id = rerouted_id
                maker = self._modules.get(id)
                logger.debug(f"Router auto-mapped speech to {id}")
        # ---------- end auto-map ----------
        ...
        if maker:
            ...
            if cmd == "notify":
                volley = Volley(
                    rcr,
                    device_id=device_id,
                    robot_data=volley_data,
                    local_data=sess.local_data,
                    data_only=True,
                )
    ...
    def handled_global(self, device_id, volley):
        global_functor = self.check_global(volley)
        if global_functor:
            logger.debug("Global response inside active module")
