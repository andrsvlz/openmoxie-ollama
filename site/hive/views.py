...
def _stt_base(url: str) -> str:
    return url.rsplit("/stt", 1)[0] if url and "/stt" in url else url

def _list_local_models(stt_url: str, timeout=3):
    base = _stt_base(stt_url)
    if not base:
        return []
    try:
        r = requests.get(f"{base}/control/models", timeout=timeout)
        r.raise_for_status()
        j = r.json()
        models = j.get("models", []) or []
        if not models:
            logger.info("STT /control/models returned 0 models from %s", base)
        return models
    except Exception as e:
        logger.warning("STT model list failed from %s: %s", base, e)
        return []
...
class SetupView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if curr_cfg:
            ...
            # Populate available local models for the dropdown if backend is local
            try:
                if curr_cfg and (curr_cfg.stt_backend or "").lower() == "local" and curr_cfg.stt_url:
                    context["stt_models"] = _list_local_models(curr_cfg.stt_url)
                else:
                    context["stt_models"] = []
            except Exception:
                context["stt_models"] = []
...
def _reload_local_stt(cfg):
    # Hot-reload only when using local backend
    if getattr(cfg, "stt_backend", "local") != "local":
        return
    url = getattr(cfg, "stt_url", "") or ""
    base = _stt_base(url)
    if not base:
        return
    payload = {
    ...
    try:
        ...
        logger.info("STT hot-reload requested: %s", payload)
    except Exception as e:
        logger.warning("STT hot-reload failed: %s", e)
...
def hive_configure(request):
    ...
    # NEW: STT toggle
    stt_backend = request.POST.get("stt_backend", "").strip().lower()
    stt_url     = request.POST.get("stt_url", "").strip()
    stt_lang    = request.POST.get("stt_lang", "").strip() or "en"
    if stt_backend in ("local", "openai"):
        cfg.stt_backend = stt_backend
    if stt_url:
        cfg.stt_url = stt_url
    cfg.stt_lang = stt_lang

    # Optional device/compute/model
    stt_model = request.POST.get("stt_model", "").strip()
    stt_device = request.POST.get("stt_device", "").strip().lower()
    stt_compute = request.POST.get("stt_compute", "").strip().lower()
    ...
    # If local backend, request hot reload so changes apply immediately
    try:
        _reload_local_stt(cfg)
    except Exception as e:
        logger.warning("Could not hot-reload STT: %s", e)
